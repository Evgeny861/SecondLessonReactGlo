...
const Features = () => {
    ...
    const arr = [
        { img: 1, title: 'Первое целевое преимущество'},
        { img: 2, title: 'Второе целевое преимущество'},
        { img: 3, title: 'Третье целевое преимущество'},
        { img: 4, title: 'Четвертое целевое преимущество'}
    ]

    const output = arr.map(item => <FeaturesSliderItem style={{ backgroundImage: `url(./images/${item.img}.svg)` }} 
                                                    text={item.title}
                                                    key={item.img} />);
    return (
        <>
